...
//            		conf.addResource(new Path("/usr/local/hadoop-1.0.3/conf/mapred-site.xml"));
	...
	public void write(HInfoWritable info, String path) throws IOException {
	 ...
	 public void hInfoReader(String basepath) throws IOException{
		 FileSystem fs = FileSystem.get(conf);
         ...
         for (int i=0;i<status.length;i++){
        	 if(!status[i].isDir()){
        		 ...
        		 CustomReader reader = new CustomReader(din,new byte[]{'$','$','$'});
        		 HInfoWritable info = new HInfoWritable(64 * 1024);
        		 int j=0,k=0;
        		 do{        			 
        			 try{
